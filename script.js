...
const dictionary = {
    ...
    "loopback": "<strong>Loopback Interface (127.0.0.1 or ::1):</strong> A fake, internal IP address. Pinging it tests if your computer's own internal networking software is functioning.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 2. Click event for Dictionary Terms
    ...
    // 3. INJECT "SHOW ANSWER" & "SHOW EXPLANATION" BUTTONS TO EVERY QUESTION
    document.querySelectorAll('.question-card').forEach(card => {
        ...
        ansBtn.addEventListener('click', () => {
            ...
            // Disable radio inputs so you can't cheat after revealing
            card.querySelectorAll('input').forEach(input => input.disabled = true);

            // Grade this specific question
            if (selectedInput) {
                ...
                // If they didn't guess, just reveal the correct answer
                const correctLabel = card.querySelector(`input[value="${correctVal}"]`).parentElement;
                if(correctLabel) correctLabel.classList.add('correct');
            }
            
            // Disable the "Show Answer" button once used
            ansBtn.disabled = true;
            ansBtn.style.opacity = '0.5';
            ansBtn.style.cursor = 'not-allowed';
            ansBtn.innerText = 'Answer Revealed';
        });
        ...
        expBtn.addEventListener('click', () => {
            ...
            if (expBox.classList.contains('show')) {
                ...
                expBtn.innerText = 'Hide Explanation';
            }
        });
    });

    // 4. GLOBAL EXAM GRADING LOGIC (The "Check Results" button at the very bottom)
